...
const Beat = styled.div`
    ...
    border-style: ${props => (props.index) % props.mode === 0 ? 'solid' : 'dashed'};
...
function Instrument(props) {
    const { sounds, selectedSound, mode, onChangeSound, beats = [], onToggleBeat, currentBeat, playing } = props;
    return (
        <StyledInstrument>
            <InstrumentLabel>{selectedSound.toUpperCase()}</InstrumentLabel>
            <Icon src={Swap} onClick={() => {
                onChangeSound(getNextSound(sounds, selectedSound))
            }} />
            <Grid>
                {beats.map((beat, index) => <Beat index={index} mode={mode} isActive={beat} currentBeat={currentBeat} playing={playing} onClick={() => onToggleBeat(index)} />)}
            </Grid>
        </StyledInstrument>
    )
}
